...
function renderResult(data) {
  ...
  const labels = { client: "Cliente", cpf: "CPF/CNPJ", osNumber: "Número da OS", contractId: "ID / Contrato", login: "Login", service: "Serviço", description: "Descrição", date: "Data" };
  ...
  document.getElementById("previewBody").innerHTML = (data.preview || []).map(row => {
    ...
    return `<tr><td>${escapeHtml(row.rowNumber)}</td><td>${escapeHtml(row.client || "—")}</td><td>${escapeHtml(row.cpf || "—")}</td><td>${escapeHtml(identification)}</td><td>${escapeHtml(row.login || "—")}</td><td>${escapeHtml(row.service || "—")}</td></tr>`;
  }).join("");
}
...
importButton.addEventListener("click", async () => {
  ...
  try {
    ...
    const data = await localApiFetch("/api/planilha/importar", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ fileName: file.name, dataBase64 }) });
